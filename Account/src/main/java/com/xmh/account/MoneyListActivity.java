...
import com.xmh.account.SellListActivity.OnFallnameClickListener;
import com.xmh.account.SellListActivity.OnKeyChangedListener;
import com.xmh.account.adapter.MoneyListAdapter;
import com.xmh.account.bean.Goods;
import com.xmh.account.service.MoneyService;
...
public class MoneyListActivity extends Activity{
	...
	private void InitView() {
		...
		lv_money.setOnItemClickListener(new OnMoneyListItemClickListener());
	...
	private void showList() {
		...
		case Application.LIMIT_DAY:
			moneylist=new MoneyService().moneyListDay(currentDate);
			btn_find.setText(DateFormat.format("yyyy-MM-dd", currentDate));
			break;
		case Application.LIMIT_MONTH:
			moneylist=new MoneyService().moneyListMonth(currentDate);
			btn_find.setText(DateFormat.format("yyyy-MM", currentDate));
			break;
		case Application.LIMIT_TOTAL:
			moneylist=new MoneyService().moneyList();
			btn_back.setVisibility(View.GONE);
			btn_find.setVisibility(View.GONE);
			btn_forward.setVisibility(View.GONE);
			break;
		}
		lv_money.setAdapter(new MoneyListAdapter(this, moneylist));
	}
	
	private void showList(String key) {
		switch(limit){
		case Application.LIMIT_DAY:
			moneylist=new MoneyService().moneyListDay(currentDate,key);
			btn_find.setText(DateFormat.format("yyyy-MM-dd", currentDate));
			break;
		case Application.LIMIT_MONTH:
			moneylist=new MoneyService().moneyListMonth(currentDate,key);
			btn_find.setText(DateFormat.format("yyyy-MM", currentDate));
			break;
		case Application.LIMIT_TOTAL:
			moneylist=new MoneyService().moneyList(key);
			btn_back.setVisibility(View.GONE);
			btn_find.setVisibility(View.GONE);
			btn_forward.setVisibility(View.GONE);
			break;
	...
	class OnBackListener implements OnClickListener{
		...
		public void onClick(View view) {
			...
			case Application.LIMIT_MONTH:
				...
				break;
			case Application.LIMIT_TOTAL:
				Toast.makeText(MoneyListActivity.this, "当前为全部信息", Toast.LENGTH_LONG).show();
				break;
			}
	...
	class OnMoneyListItemClickListener implements OnItemClickListener{
	...
	class OnSureRemarksListener implements OnClickListener{
		...
		public void onClick(View view) {
			...
			if(new MoneyService().updateRemarks(goods)){
				Toast.makeText(MoneyListActivity.this, "已更新备注", Toast.LENGTH_LONG).show();
			}
			ll_remarks_input.setVisibility(View.GONE);
			showList();
		}
