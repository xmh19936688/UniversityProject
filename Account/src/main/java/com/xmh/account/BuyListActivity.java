...
import com.xmh.account.StockActivity.OnSureRemarksListener;
import com.xmh.account.adapter.BuyListAdapter;
...
public class BuyListActivity extends Activity{
	...
	private void InitView() {
		...
		lv_buy.setOnItemClickListener(new OnBuyListItemClickListener());
	...
	private void showList() {
		...
		case Application.LIMIT_DAY:
			buylist=new BuyService().buyListDay(currentDate);
			btn_find.setText(DateFormat.format("yyyy-MM-dd", currentDate));
			break;
		case Application.LIMIT_MONTH:
			buylist=new BuyService().buyListMonth(currentDate);
			btn_find.setText(DateFormat.format("yyyy-MM", currentDate));
			break;
		case Application.LIMIT_TOTAL:
			buylist=new BuyService().buyList();
			btn_back.setVisibility(View.GONE);
			btn_find.setVisibility(View.GONE);
			btn_forward.setVisibility(View.GONE);
			break;
		}
		lv_buy.setAdapter(new BuyListAdapter(this, buylist));
	}
	
	private void showList(String key) {
		switch(limit){
		case Application.LIMIT_DAY:
			buylist=new BuyService().buyListDay(currentDate,key);
			btn_find.setText(DateFormat.format("yyyy-MM-dd", currentDate));
			break;
		case Application.LIMIT_MONTH:
			buylist=new BuyService().buyListMonth(currentDate,key);
			btn_find.setText(DateFormat.format("yyyy-MM", currentDate));
			break;
		case Application.LIMIT_TOTAL:
			buylist=new BuyService().buyList(key);
			btn_back.setVisibility(View.GONE);
			btn_find.setVisibility(View.GONE);
			btn_forward.setVisibility(View.GONE);
			break;
	...
	class OnBackListener implements OnClickListener{
		...
		public void onClick(View view) {
			...
			case Application.LIMIT_MONTH:
				...
				break;
			case Application.LIMIT_TOTAL:
				Toast.makeText(BuyListActivity.this, "当前为全部信息", Toast.LENGTH_LONG).show();
				break;
			}
	...
	class OnBuyListItemClickListener implements OnItemClickListener{
	...
	class OnSureRemarksListener implements OnClickListener{
		...
		public void onClick(View view) {
			...
			if(new BuyService().updateRemarks(goods)){
				Toast.makeText(BuyListActivity.this, "已更新备注", Toast.LENGTH_LONG).show();
			}
			ll_remarks_input.setVisibility(View.GONE);
			showList();
		}
